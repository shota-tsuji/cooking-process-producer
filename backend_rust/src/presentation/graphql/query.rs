...
impl Query {
    ...
    async fn recipes(&self, _ctx: &Context<'_>) -> Result<Vec<Recipe>, String> {
        let rs: Vec<db_entity::recipes::Model> = db_entity::recipes::Entity::find()
            ...
            .await
            .map_err(|e| e.to_string())?;
        let recipes = rs.into_iter().map(|recipe| Recipe {
            id: recipe.id,
            title: recipe.title,
    ...
    async fn process(&self, _ctx: &Context<'_>, id: ID) -> Result<Process, String> {
        ...
        let recipe_models: Vec<db_entity::recipes::Model> = db_entity::recipes::Entity::find()
            ...
            .await
            .map_err(|e| e.to_string())?;
        let recipes: Vec<Recipe> = recipe_models.iter().map(|recipe| {
            Recipe {
                id: recipe.id.clone(),
        ...
        for recipe in &recipes {
            let step_models: Vec<db_entity::steps::Model> = db_entity::steps::Entity::find()
                ...
                .await
                .map_err(|e| e.to_string())?;
            let steps: Vec<Step> = step_models.iter().map(|step| {
                Step {
                    id: step.id.clone(),
        ...
        let mut resource_models: Vec<db_entity::resources::Model> = db_entity::resources::Entity::find()
            .all(&self.db)
            .await
            .map_err(|e| e.to_string())?;
        let mut resources: Vec<Resource> = resource_models.iter().map(|resource| {
            Resource {
                id: resource.id,
    ...
    async fn resource(&self, _ctx: &Context<'_>, id: ID) -> Result<Resource, String> {
        let model: db_entity::resources::Model = db_entity::resources::Entity::find_by_id(id.as_str().parse::<u64>().unwrap())
            .one(&self.db)
            .await
            .map_err(|e| e.to_string())?
    ...
    async fn resources(&self, _ctx: &Context<'_>) -> Result<Vec<Resource>, String> {
        let models: Vec<db_entity::resources::Model> = db_entity::resources::Entity::find()
            ...
            .await
            .map_err(|e| e.to_string())?;
        let resources = models.into_iter().map(|resource| Resource {
            id: resource.id,
            name: resource.name,
            amount: resource.amount,
